...
class Redis{
    ...
    protected static function init(){
        if(self::$connect==false){
            // 只有一台 Redis 的应用
            self::$connect = new Redisa();
            self::$connect -> connect(['host'=>'127.0.0.1','port'=>6379]);
        }
    }
...
class Redisa{
    ...
    public function connect($config=array('host'=>'127.0.0.1','port'=>6379), $isMaster=true){
        ...
        // 设置 Master 连接
        if($isMaster){
            $this->_linkHandle['master'] = new Predis();
            $ret = $this->_linkHandle['master']->pconnect($config['host'],$config['port']);
        }else{
            // 多个 Slave 连接
            $this->_linkHandle['slave'][$this->_sn] = new Predis();
